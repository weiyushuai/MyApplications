...
public class PagerTab extends ViewGroup {
	...
	private int mTabBackgroundResId = 0;// tab背景资源
	private int mTabTextColorResId = 0; //tab文字颜色
	...
	private void init() {
		...
		//创建一个scroller
		mScroller = ScrollerCompat.create(ct);
		//获取一个系统关于View的常量配置类
		final ViewConfiguration configuration = ViewConfiguration.get(ct);
		//获取滑动的最小距离
		...
		mLeftEdge = new EdgeEffectCompat(ct);
		mRightEdge = new EdgeEffectCompat(ct);
	}
	...
	private void addIconTab(final int position, int resId) {
		ImageButton tab = new ImageButton(ct);
